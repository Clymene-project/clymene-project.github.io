---
title: clymene-config
sort_rank: 1
---

Clymene is configured via command-line flags and a configuration file.

To view all available command-line flags, run `./clymene-agent -h`.

Clymene-agent can reload its configuration at runtime. If the new configuration
is not well-formed, the changes will not be applied.
A configuration reload is triggered by sending a HTTP POST request to the `/api/reload` endpoint.
